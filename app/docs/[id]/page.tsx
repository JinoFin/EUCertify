    ...
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10">
      ...
      <Tabs.Root defaultValue={initialTab} className="space-y-4">
        ...
        <Tabs.Content value="DOC">
          <DocPreview docId={`${params.id}-doc`} productId={params.id} type="DOC" />
        </Tabs.Content>
        <Tabs.Content value="CHECKLIST">
          <DocPreview docId={`${params.id}-checklist`} productId={params.id} type="CHECKLIST" />
        </Tabs.Content>
        <Tabs.Content value="LABEL">
          <DocPreview docId={`${params.id}-label`} productId={params.id} type="LABEL" />
        </Tabs.Content>
      </Tabs.Root>
    </section>
  );
