...
const renderValue = (field: DocTemplate['fields'][number], value: any) => {
  switch (field.type) {
    case 'checkbox':
      return value ? tDoc('doc.field.yes', 'Yes') : tDoc('doc.field.no', 'No')
    case 'multiselect':
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={columns.length || 1} className="doc-empty">
                  {tDoc('doc.table.empty', 'No entries yet')}
                </td>
              </tr>
            ) : (
              rows.map((row, index) => (
    ...
    <div className="doc-renderer">
      <header className="doc-header">
        ...
        {template.exportable.includes('pdf') && (
          <span className="doc-badge" aria-label={tDoc('doc.badge.exportable', 'Generated by EUCertify')}>
            {tDoc('doc.badge.exportable', 'Generated by EUCertify')}
          </span>
        )}
      </header>
      <section className="doc-meta">
        <div>
          <strong>{tDoc('doc.meta.version', 'Version')}</strong>
          <span>{instance.version}</span>
        </div>
        <div>
          <strong>{tDoc('doc.meta.created', 'Created')}</strong>
          <span>{formatDate(instance.createdAt)}</span>
        </div>
        <div>
          <strong>{tDoc('doc.meta.updated', 'Updated')}</strong>
          <span>{formatDate(instance.updatedAt)}</span>
        </div>
        <div>
          <strong>{tDoc('doc.meta.status', 'Status')}</strong>
      ...
      {template.footerNotes?.length ? (
        <footer className="doc-footer">
          <h3>{tDoc('doc.footer.notes', 'Notes')}</h3>
          <ul>
